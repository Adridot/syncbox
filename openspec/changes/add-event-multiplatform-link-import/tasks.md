...
- [ ] 7.3 Run `pnpm --dir ui test` and `pnpm --dir ui build`, then verify the packaged event workflow for single tracks and albums/playlists on all four providers using authorized samples; record actual outcomes rather than treating upstream support as a successful packaged test.
