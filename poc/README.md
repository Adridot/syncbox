...
local one-shot Premium credential. Phase 7 revalidated the exact final local
bytes for POC #1, #2, #3, #5, and #6, including two-root equality and the
post-runtime scanner. B2 purchase links remain the primary v1 missing-track
path; B1 is optional and disabled by default.

| # | POC | Current evidence | State | GO condition | Fallback |
|---|---|---|---|---|---|
| 1 | Sidecar process lifecycle on macOS | Source, frozen, app-embedded, optional source/frozen/packaged, single-instance, foreign/stale listener, immediate-failure, and supervisor exhaustion/recovery lanes pass on the exact reproducible bytes. Graceful, TERM, and KILL leave no orphan and release ports 8765/8766. | **GO on exact local final bytes; public download gate pending** | Keep every lane green on the public download. | Block the artifact on regression; retain the native supervisor until measured evidence requires a change. |
| 2 | PyInstaller onedir size and cold start | The final-candidate base ZIP is 29,296,019 bytes (`296fbece…`); the optional ZIP is 17,340,644 bytes (`13976d4b…`). Two clean absolute roots produced byte-identical ZIPs and unpacked trees, and both strict scanners pass. | **GO on reproducible local bytes; public download gate pending** | Preserve both exact hashes and validate the public downloads. | Keep onedir; investigate another freezer only after a measured failure. |
| 3 | SSE in the real macOS Tauri WebView | The exact final WKWebView navigation and normal-quit walkthrough passed; UI completion/reconnect tests pass. Real packaged Spotify PKCE, refresh, forged-state rejection, invalid-grant recovery, listener shutdown, and port release pass with permanent SSE/API on 8766 and temporary callback on 8765. | **GO on exact local final bytes; public download gate pending** | Keep packaged completion/reconnect and OAuth port behavior green on downloaded bytes. | Investigate WebKit caching/buffering first; change transport only with owner approval. |
...
| 5 | Deezer full-track streamrip | [Phase 5](../docs/_handoffs/phase-05-b1-acquisition.md) proved a full 337.56-second track with a one-shot local Premium credential. The final candidate then passed real artwork embedding through source, the exact frozen component, base-boundary installation, and packaged-app lanes. Each lane verified the same embedded JPEG and cleaned all credential/output state. | **GO on local final-candidate bytes; public download gate pending** | Keep the exact optional hash and repeat installation/runtime validation after public download. | Keep B2 primary. If the public asset fails validation, remove it; the base remains functional. |
