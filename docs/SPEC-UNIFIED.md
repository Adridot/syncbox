...
| Périmètre produit (OVERHAUL-01, 2026-06-16) | **Intégré** (§4, §5.11–§5.13, §6.5, §6.12, §7.4, §8). macOS v1 includes A1 Smart Fixes, A3 faux-320/FLAC, and B2 Track Matcher légal (+ D7 déjà spécifié §5.8). B1 streamrip is deferred to v1.1 after the Phase 5 `BLOCKED` gate unless that gate is rerun and returns `GO`. **Gate 2 (2026-06-16)** : A2 dedup-empreinte (Chromaprint) **différée v2** (résiduel étroit + binaire LGPL, POC-gated) ; SoundCloud **différé v2/B4** (tire ffmpeg). Recherche : [_research/11–14](_research/) |

---

## 1. Identité produit & résumé

Syncbox est une app desktop **macOS + Windows** (Linux exclu : Rekordbox n'y tourne pas) qui maintient la collection **Rekordbox** d'un DJ : elle synchronise des **playlists Spotify** (lecture seule), **entretient la collection** (doublons, fichiers manquants, tags, **nettoyage de métadonnées en masse**, **détection faux-320/FLAC**), et propose pour les morceaux manquants un **chemin légal** mis en avant (liens d'achat Beatport/Bandcamp, sans réseau côté app). The B1 Deezer/streamrip downloader is not part of the macOS v1 application after the Phase 5 `BLOCKED` gate. Open-source.
...
- B1 Deezer/streamrip acquisition is deferred from the macOS v1 application. Phase 5 ended `BLOCKED`, not `GO`, because no local Premium Deezer ARL was available for the required full-track POC. The v1 base app has no acquisition job, Deezer ARL setting, streamrip dependency, acquisition API, or acquisition UI. Those sections describe a v1.1 candidate only unless the Phase 5 gate is rerun and returns `GO`.
...
Tokens OAuth Spotify **jamais en clair** (incompatible open-source). Voir §6.7 (deux chemins documentés). If B1 is re-opened after a future `GO`, the Deezer ARL follows the same rule and no acquisition library may serialize it to disk.
...
- **Deferred B1 acquisition job** — not present in the macOS v1 application after Phase 5 `BLOCKED`. If a future gate returns `GO`, the candidate model remains 3 scopes (`event`/`library`/`collection`) unified in one global job with `pending → resolved → queued → downloading → downloaded → ready` and `acquisition_failed`/`acquisition_ambiguous` failure states.
...
- **Réglages** — Spotify credentials, 4 paths, `backup_retention`, OAuth tokens (encrypted, §6.7). The base macOS v1 app has no Deezer ARL setting.
...
### 6.5 Acquisition — B2 in macOS v1, B1 deferred

**Current macOS v1 path:** **Chemin légal (B2, par défaut, §5.13)** — Track Matcher : liste les manquants + **liens d'achat** Beatport/Bandcamp (URL de recherche, **stdlib, zéro réseau côté app**). Mis en avant comme alternative ToS-propre.

**Deferred B1 candidate:** the optional Deezer download module stays out of the macOS v1 app after the Phase 5 `BLOCKED` gate. Re-open it only if a local Premium ARL is available and the full-track POC returns `GO`. If that happens, the user must explicitly enable it and provide the ARL; it remains isolated behind `DeezerAcquirer.download(track_id) -> Path`, pinned, and never on the critical `master.db` write path.
...
**Porte bloquante (sourcée, POC #6).** L'API officielle Deezer limite à **30 s de preview** ; la viabilité du **téléchargement full-track** is still unproven after Phase 5 because no local Premium ARL was available. A future rerun must prove it with a real Premium ARL on Apple Silicon before any B1 code, dependency, setting, job, API, or UI is added. Télécharger **par ID numérique** (résolu depuis l'ISRC), jamais par URL courte (#865).
...
>
> `ponytail:` **B1 is deferred from macOS v1 after the Phase 5 `BLOCKED` gate.** B2 (legal path, zero app-side network and zero bundle weight) is the only v1 missing-track path. Re-open B1 in v1.1 only after POC #6 returns `GO` with a local Premium ARL and the licensing/package boundary is revalidated.
...
**Conditions dures (sourcées, [07_Secrets](_research/07_Secrets-au-repos.md)).** keyring **dépend d'un Developer ID stable** : sur macOS Tahoe, un binaire PyInstaller non signé Apple reçoit `errSecInteractionNotAllowed -25308` **même keychain déverrouillé** (cause = code-signing) ; et sans identité stable, **chaque release invalide l'ACL** → re-prompt. Donc : **signé → keyring** ; **non signé → store chiffré**. Dans tous les cas : **ne jamais écrire de secret en clair** ; chiffrer seulement the real v1 secrets, currently Spotify OAuth tokens. If B1 is reopened after a future `GO`, the Deezer ARL must use the same secret path and must never appear in a streamrip `config.toml`. Do not encrypt the entire DB; **figer le chemin d'extraction** (onedir ou `--runtime-tmpdir` stable). Côté Windows, DPAPI fonctionne tel quel.
...
| B1 backend streamrip (Deezer-only) | **DEFERRED v1.1** after Phase 5 `BLOCKED`: no Premium ARL was available for the required full-track POC, so no B1 app code, dependency, setting, job, API, UI, or release claim is authorized for macOS v1. SoundCloud → v2/B4 | §5.5, §6.5, §7.1 Fork D, [_research/14](_research/14_streamrip-embedding-Deezer-SoundCloud.md), [_handoffs/phase-05-b1-acquisition.md](_handoffs/phase-05-b1-acquisition.md) |
...
6. **Acquisition — B1 deferred gate** : Phase 5 is `BLOCKED`, not `GO`, because no local Premium ARL was available. A future v1.1 rerun must validate **full-track Deezer download with a real Premium ARL** (vs preview 30 s) via **streamrip lib** (pin git v2.2.0, exact SHA locked), by numeric Deezer ID resolved from ISRC; no B1 app code is authorized before that evidence. SoundCloud/ffmpeg **hors v1**.
