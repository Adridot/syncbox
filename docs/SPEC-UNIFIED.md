...
**Porte validée (POC #6).** The Phase 5 rerun proved full-track Deezer download on macOS Apple Silicon with a real Premium ARL: ISRC `USQX91300105`, Deezer track id `67238732`, catalogue duration `337 s`, measured duration `337.56 s`, `track.download_path` source, `13,520,081` bytes, and output cleanup. B1 downloads by numeric Deezer ID resolved from ISRC, never by short URL (#865). Phase 6 must still validate the packaged optional-component installer and prove the base bundle contains no streamrip/GPL code or Deezer ARL.
...
| **D — Acquisition** | **Module OPTIONNEL, OFF par défaut** + ARL utilisateur ; **chemin légal B2 (liens d'achat) mis en avant**. Lib = **streamrip** as a separate optional component (pin git v2.2.0 + exact commit), **Deezer-only en v1** ; **deemix-fork = fallback documenté** ; **SoundCloud → v2/B4** (ffmpeg). | **Tranché** (Phase 5 full-track POC #6 = `GO`; Phase 6 validates packaged component boundary) | §6.5 |
...
| 10.2 | Acquisition Deezer | Module optionnel **OFF par défaut** ; lib = **streamrip tranchée** as a separate optional component (pin git v2.2.0 + exact commit; fallback deemix-fork documenté). Phase 5 full-track Deezer POC #6 returned `GO`; Phase 6 must validate the packaged installer and base-bundle exclusion. §6.5 |
...
- **À confirmer en POC** (non vérifiables en lecture seule) : fidélité pyrekordbox RB 7.x ; packaged optional streamrip component install + base-bundle exclusion; chemins `master.db` Windows ; cold-start/taille réels (avec `miniaudio`/A3) ; signature sidecar end-to-end mac+Win ; calibration A3 + branchement A3→D6 ; robustesse des templates d'URL B2 ; (avant v2) volume réel de doublons ratés justifiant A2.
