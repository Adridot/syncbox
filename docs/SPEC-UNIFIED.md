...
| Product scope (OVERHAUL-01, 2026-06-16) | **Integrated** (§4, §5.11–§5.13, §6.5, §6.12, §7.4, §8). macOS v1 includes A1 Smart Fixes, the conservative A3 spectral fallback, optional B1 Deezer/streamrip acquisition, and B2 legal Track Matcher. Full A3 classification is `NO-GO`: the fallback reports `ok` or `incertain` and never penalizes keeper selection. B1 returned `GO` on 2026-07-13 after a real full-track Deezer POC. A2/Chromaprint remains deferred. Event web-audio support is approved separately; see §5.7 and its release gates. |
...
- Event YouTube/SoundCloud acquisition and a focused FFmpeg runtime are approved only in a separately installed web-audio component. Distribution remains gated on complete notices and packaged verification. AcoustID/MusicBrainz enrichment, automatic cues, beatgrid editing, a cloud backend, and a mobile app remain deferred.
...
For the separately packaged event web-audio component, the owner approved on 2026-09-07: the focused FFmpeg/ffprobe 9.0.1 LGPL-2.1-or-later build, LAME 3.100 encoder under LGPL-2.0-or-later, and Deno 2.9.5 with cooked-waker 5.0.0, smartstring 1.0.1, webpki-root-certs 0.26.6, webpki-roots 0.26.1, cssparser 0.36.0, cssparser-macros 0.6.1 and dtoa-short 0.3.5 under their inventoried MPL terms. This approval is scoped to the web-audio component and still requires complete generated notices and exact sources before distribution. It does not imply the feature is already released.
...
**5.7 Events.** 3 modes (from playlist / empty / by link). Unique atomic folder (`mkdir(exist_ok=False)`, slug collision → `-2`…). `default_tag` = event name (“Situation” category). Event matching: `ambiguous`→`ambiguous` (not `conflict`), no default tags. Staging/claim for Spotify/manual rows: one file shared only between two tracks with the **same non-empty ISRC**. Direct provider links require verified same-item provenance or an explicit local-file association instead. Apply: creates/repairs a **smart playlist** under “Event Imports”, restores the XML after commit. **Smart playlist**: `SmartList = "<playlistId>:<tagId>"` (operator 8 = contains); IDs > 2³¹ converted to **signed 32-bit** — **load-bearing**. RB write: new rows with **string ID** (mixed int+string PKs crash SQLAlchemy on flush); self-heal of a soft-deleted artist. Delete preview read **inside** the mutation session. **D10**: remove Live Import M3U8 (UI + `live_import.py` + route); always require RB closed and write the collection directly (eliminates B12). **D11/D23**: delete event **always with exact preview** + **guarded on `mutationAllowed`** like apply (fixes B11). **Cleanup (fixes T8/T12)**: event deletion removes its **disk artifacts** (staging folder, audio, `.xml.bak` snapshots) — no orphans.

**Event link snapshots (2026-09-07).** Existing events accept Spotify/Deezer tracks, albums and playlists; YouTube videos and playlists, including YouTube Music album representations; and SoundCloud tracks and sets/albums. A video with playlist context requires an explicit scope choice. A bounded background worker resolves a persisted preview, keeping source order and disclosed unavailable entries. Confirmation atomically adds only selected entries as manual rows, deduplicates provider/item identity, and stores the result for replay. Removed/ignored items require explicit re-addition. Preview, dismissal and confirmation create no audio jobs and never write Rekordbox. Imports do not change the event's synchronized Spotify playlist.

Direct Deezer/YouTube/SoundCloud rows preserve the exact linked recording through matching, staging, acquisition, restart, apply and managed-file migration. Similar titles or matching ISRCs do not establish identity. Automatic reuse requires recorded same-item provenance plus a matching file hash. Explicit local selection records the selected path and hash. Spotify retains existing ISRC/fuzzy matching and automatic/manual Deezer association. Each acquisition job stores its provider, server-owned selector, effective item, actual final path, source quality, output properties and processing. The shared FIFO continues after per-item failures; published-output recovery verifies owner, selector and hash. Web audio requires its own enablement/component, independently of Deezer credentials. Spotify disconnect removes Spotify previews/provenance while preserving direct-source jobs and imports.

Previews are bounded to 1,000 entries, 100 catalogue pages, a 4 MiB stored manifest and a 120-second resolution budget; provider access failures and limits are explicit. Authenticated Spotify track hydration uses bounded individual requests with partial-result preservation and the existing anonymous title-only fallback. Spotify collections require a connected account with access to their items. Web acquisition prefers original/native supported audio, remuxes when possible, otherwise converts to MP3; complete short audio needs no artwork. Source and output quality are displayed separately. Ordinary Rekordbox apply/reapply guards remain mandatory.
...
**Event web audio (approved 2026-09-07):** YouTube, YouTube Music and public SoundCloud metadata/audio use a separate component containing pinned yt-dlp/EJS, Deno, and focused audio-only FFmpeg/LAME. The base sidecar imports none of these dependencies. Metadata previews never download audio. The complete Deno/native license inventory was verified on 2026-09-07; distribution still requires publishing the packaged archive as the release asset named in its manifest.
...
| **D — Acquisition** | **Optional module, OFF by default**, with B2 purchase links kept primary. streamrip is a separately distributed component pinned to v2.2.0 and an exact commit; the library/collection acquisition interface remains Deezer-only. Events additionally have approved exact-source Deezer/YouTube/SoundCloud selectors and a separate web-audio component, pending its distribution gates. Deemix remains a documented fallback only. | **Resolved** (full-track, artwork, packaged-boundary, notice, publication, and public download-back gates pass) | §6.5 |
...
| B1 streamrip backend (Deezer-only) | **ADD v1** after Phase 5 `GO`: optional OFF by default, exact pinned component, artwork support, encrypted one-shot ARL, and no Deezer runtime in the base. Event web audio is approved separately, subject to release gates | §5.5, §6.5, §7.1, _handoffs/phase-05-b1-acquisition.md (archived) |
...
6. **Acquisition B1** — `GO` for the full-track, artwork, and packaged boundary. Source, exact frozen, installed, and packaged lanes verify the embedded cover. FFmpeg remains outside the base and Deezer bundles; the separate event web-audio artifact has its own notice and packaged-workflow gates.
