...
| Périmètre produit (OVERHAUL-01, 2026-06-16) | **Intégré** (§4, §5.11–§5.13, §6.5, §6.12, §7.4, §8). **v1 = 4 ajouts** : A1 Smart Fixes, A3 faux-320/FLAC, **B1 streamrip (Deezer-only, v1-*conditionnel* — POC #6, cf. §7.4)**, B2 Track Matcher légal (+ D7 déjà spécifié §5.8). **v1 *ferme* (0 risque) = A1 + B2** ; B1 et A3 portent un repli documenté. **Gate 2 (2026-06-16)** : A2 dedup-empreinte (Chromaprint) **différée v2** (résiduel étroit + binaire LGPL, POC-gated) ; SoundCloud **différé v2/B4** (tire ffmpeg). Recherche : [_research/11–14](_research/) |
| Amendements post-design (**Gate 3**, 2026-07-02) | **Intégrés** (§11) : SoundCloud **v1 borné** à l'ajout manuel de titre à un event (COMMENT POC-gated, amende Gate 2) ; **cycle « modifié → ré-appliquer »** des events ; **ajout de titre par lien** (Spotify/Deezer/SoundCloud) ; **readouts de collection** dashboard dérivés du snapshot (zéro moteur) ; micro-décisions UI entérinées. Design tranché : [SPEC-DESIGN.md](SPEC-DESIGN.md) + mockup [`Syncbox.dc.html`](../syncbox-ui-ux-design/project/Syncbox.dc.html) |

---

## 1. Identité produit & résumé

Syncbox est une app desktop **macOS + Windows** (Linux exclu : Rekordbox n'y tourne pas) qui maintient la collection **Rekordbox** d'un DJ : elle synchronise des **playlists Spotify** (lecture seule), **entretient la collection** (doublons, fichiers manquants, tags, **nettoyage de métadonnées en masse**, **détection faux-320/FLAC**), et propose deux chemins pour les morceaux manquants : un **chemin légal** mis en avant (liens d'achat Beatport/Bandcamp, sans réseau côté app) et un **module de téléchargement optionnel, OFF par défaut** (Deezer via streamrip). Open-source.
...
Tokens OAuth Spotify + ARL Deezer **jamais en clair** (incompatible open-source). Voir §6.7 (deux chemins documentés). **Clause dure (lib tierce d'acquisition)** : aucune lib d'acquisition (streamrip, deemix-fork) ne **sérialise l'ARL sur disque** — `Config` construit **en mémoire par job** (`Config.defaults()` patché, jamais `from_file`/`save`), dossier de config neutralisé ; **garantie testée** qu'aucun `config.toml` contenant l'ARL n'est créé (le défaut streamrip écrit sinon l'ARL en clair).
...
- **Job d'acquisition** — 3 scopes (`event`/`library`/`collection`), unifiés en un job global. Cycle : `pending → resolved → queued → downloading → downloaded → ready` ; échecs `acquisition_failed`/`acquisition_ambiguous`. **Acquisition Deezer** — **unique provider v1, Deezer implicite ; aucun champ/registre provider en v1** (l'introduction d'un champ provider est une décision v2 avec SoundCloud/B4, cf. §6.5). Moteur **streamrip** (lib, pin git), interface mince ; chemin de sortie réel (D18), `Config` par job (zéro état global, §5.5).
...
- **Réglages** — credentials Spotify, ARL Deezer, 4 chemins, `backup_retention`, tokens OAuth (chiffrés, §6.7).
...
- **Moteur (B1, §6.5)** : **streamrip** importé comme **lib** (pin git **v2.2.0**, PAS PyPI 2.1.0 cassé), derrière l'interface mince `DeezerAcquirer.download(track_id) -> Path` (séquence d'appels API exacte reléguée au §6.5/POC #6 — pas répétée ici). Le **vrai chemin de sortie réalise D18** (jamais reconstruit). `Config` par instance/job porte ARL + dossier → **zéro état global** (corrige F2/F3). ARL **en mémoire**, jamais écrit en clair (§3.6). **v1 = Deezer uniquement** (FLAC/MP3 servi direct, **pas de ffmpeg**) ; SoundCloud → v2/B4 (HLS, exige ffmpeg). **deemix-fork = fallback documenté** si POC #6 révèle un coût aiohttp ou une fragilité d'API interne bloquante.
...
### 6.5 Acquisition — module optionnel OFF par défaut, chemin légal mis en avant (Fork D)

**Deux chemins pour les morceaux manquants, le légal d'abord** (décision OVERHAUL-01, Gate 1) :
1. **Chemin légal (B2, par défaut, §5.13)** — Track Matcher : liste les manquants + **liens d'achat** Beatport/Bandcamp (URL de recherche, **stdlib, zéro réseau côté app**). Mis en avant comme alternative ToS-propre.
2. **Module de téléchargement (OPTIONNEL, OFF par défaut)** — l'utilisateur l'active **explicitement** et fournit son **ARL**. Limite l'**exposition DMCA** du dépôt (précédent Deezer 2021), supprime le **process Deemix externe `:6595`**. **Isolé derrière une interface mince** `DeezerAcquirer.download(track_id) -> Path`, **version pinée**, **jamais sur le chemin critique d'écriture `master.db`**.
   - **Confinement GPL — clause dure, testable** : streamrip est **GPL-3.0-only**, importé **in-process**. Le sidecar de base **n'importe JAMAIS streamrip au boot** : import **différé** derrière le flag d'activation (OFF par défaut), **garantie testée** qu'aucun symbole streamrip n'est chargé tant que le module n'est pas activé. **Exigence ferme de distribution** : le code streamrip/deemix-fork (GPL-3) **n'est PAS lié ni embarqué dans l'artefact de distribution du sidecar de base** — il est livré comme **composant séparé** téléchargé/activé à part (*mere aggregation*, pas œuvre combinée distribuée). La base distribuée reste **MIT-compatible** ; **seul le module optionnel, une fois activé, charge le code GPL-3**. La « garantie testée qu'aucun symbole n'est chargé au boot » protège le runtime ; la séparation de l'artefact protège la **distribution**. Même règle pour deemix-fork (fallback).

**Lib = streamrip (défaut), Deezer-only v1** (sources/API : [14_streamrip-embedding-Deezer-SoundCloud.md](_research/14_streamrip-embedding-Deezer-SoundCloud.md), [10_Acquisition](_research/10_Acquisition-2026.md)).
- **streamrip importé comme lib** (pin git **v2.2.0** — **PAS PyPI 2.1.0**, parser Deezer cassé ; SHA du tag à résoudre + figer au lockfile). API documentée : `Config.defaults()` par job → `client.login()` → `PendingSingle.resolve()` → `track.download()` → **`track.download_path`** (réalise **D18** sans reconstruction de nom). `Config` par instance ⇒ **zéro état global** (corrige F2/F3). Socle ARL+Blowfish (`pycryptodomex`/`mutagen` déjà bundlé). **Clause TLS dure (testable, miroir §3.6)** : garantie testée que le client HTTP de la lib d'acquisition utilise le **bundle `certifi` embarqué** (contexte SSL explicite), **jamais** un CA système absent du bundle PyInstaller, et que la **vérification TLS n'est jamais désactivée** (§3.5).
- **CLI sous-process écartée** : pas de sortie machine-lisible → forcerait la reconstruction de nom, interdite par §5.5/D18.
- **deemix-fork (vietsman) = fallback documenté** (Gate 2) : `requests` (pas d'`aiohttp`, plus léger), bascule **si** le POC #6 révèle un coût aiohttp ou une fragilité d'API interne streamrip bloquante. **Risque dominant** : l'API interne streamrip (`Track` en `slots`, non documentée hors-wiki) est fragile à l'upgrade → **pin commit strict + tests d'intégration survivant à un bump** (garde-fou maintenabilité §2).

**SoundCloud → v2/B4** : il sert du HLS MP3 et **exige ffmpeg externe** (+40-80 Mo/plateforme, packaging cross-OS §3.7) qui doublerait quasi le sidecar (§2 légèreté). Deezer (FLAC/MP3 servi direct) **n'exige pas ffmpeg** → v1 reste léger. SoundCloud reviendra idéalement en **plugin téléchargeable hors sidecar de base**. **⚠ Amendé Gate 3 (§11.1)** : SoundCloud entre en v1 **borné au seul ajout manuel de titre à un event** (décision propriétaire post-design) ; library/collection restent **Deezer-only**. Le COMMENT (ffmpeg système vs composant séparé) est une décision build POC-gated — jamais embarqué dans l'artefact de base.

**Porte bloquante (sourcée, POC #6).** L'API officielle Deezer limite à **30 s de preview** ; la viabilité du **téléchargement full-track** est à **prouver par un POC avec un ARL Premium réel** (macOS + Windows) — aucune source ne la garantit en 2026 (issues #846/#865 = fragilité auth/URL). Télécharger **par ID numérique** (résolu depuis l'ISRC), jamais par URL courte (#865).

> `ponytail:` ne pas réécrire l'acquisition nativement (Rust/Go) — sur-ingénierie face à des changements d'API Deezer fréquents ; une lib pinée derrière une interface mince + un chemin de mise à jour rapide suffit. Le module optionnel limite GPL/DMCA sans renoncer au « tout au même endroit ». Le chemin légal B2 (stdlib) couvre le besoin sans aucun risque ni poids.
>
> `ponytail:` **B1 est v1 mais sortable.** Sa viabilité full-track n'est pas garantie (POC #6 ; preview 30 s, issues auth #846/#865) et son API interne streamrip est fragile (`slots`). **Si le POC #6 échoue ou l'API se révèle ingérable → B1 bascule en v1.1/v2 sans bloquer la release** ; B2 (chemin légal, 0 risque/0 poids) reste alors l'**unique** voie « morceaux manquants » de v1. Condition de maintien en v1 : POC #6 vert **et** fallback deemix-fork validé. On n'écarte le module download que si le POC le condamne.
...
| **D — Acquisition** | **Module OPTIONNEL, OFF par défaut** + ARL utilisateur ; **chemin légal B2 (liens d'achat) mis en avant**. Lib = **streamrip** (défaut, importé comme lib, pin git v2.2.0), **Deezer-only en v1** ; **deemix-fork = fallback documenté** ; **SoundCloud → v2/B4** (ffmpeg). | **Tranché** (Gate 1/2 ; full-track POC-gated #6) | §6.5 |
...
| 10.2 | Acquisition Deezer | Module optionnel **OFF par défaut** ; lib = **streamrip tranchée** (pin git v2.2.0, importée comme lib derrière interface mince ; fallback deemix-fork documenté). **Seule la viabilité full-track Deezer reste POC #6** (pas le choix de lib). §6.5 |
...
| B1 backend streamrip (Deezer-only) | **CHANGER, v1-conditionnel** : entre en v1 **si POC #6 vert + fallback deemix-fork validé**, sinon différé v1.1 — **sans bloquer la release** (v1 *ferme* = A1 + B2, 0 risque ; B2 reste le chemin manquants) ; SoundCloud → v2/B4 | §5.5, §6.5, §7.1 Fork D, [_research/14](_research/14_streamrip-embedding-Deezer-SoundCloud.md) |
...
6. **Acquisition — porte bloquante** : valider le **téléchargement full-track Deezer avec un ARL Premium réel** (vs preview 30 s) via **streamrip lib** (pin git v2.2.0, **SHA du tag à résoudre + figer au lockfile**), par **ID numérique résolu de l'ISRC** (jamais d'URL courte, #865) ; packaging `pycryptodomex`(Blowfish)/`mutagen` gelé mac+Win ; **wrapper `DeezerAcquirer` via `PendingSingle.resolve()→track.download_path`** (D18 réel, jamais `Track` instancié à la main), ARL **en mémoire** sans écrire `config.toml` (§3.6), isolation par `Config`/job (F2/F3). **Bascule deemix-fork** si coût aiohttp ou fragilité d'API interne bloquante. SoundCloud/ffmpeg **hors v1**.
...
- **§10.9 — UI/UX.** Structure des écrans, navigation (routeur/deep-link ?), regroupements (centre d'acquisition unifié, hub « Santé de collection »), flux guidés/onboarding. *(Tranché depuis dans [SPEC-DESIGN.md](SPEC-DESIGN.md).)*
...
- **À confirmer en POC** (non vérifiables en lecture seule) : fidélité pyrekordbox RB 7.x ; viabilité full-track Deezer (streamrip lib, SHA tag) ; chemins `master.db` Windows ; cold-start/taille réels (avec `miniaudio`/A3) ; signature sidecar end-to-end mac+Win ; calibration A3 + branchement A3→D6 ; robustesse des templates d'URL B2 ; (avant v2) volume réel de doublons ratés justifiant A2.
...
### 11.1 SoundCloud — v1, borné à l'ajout manuel de titre à un event

**QUOI (décision propriétaire).** Le champ « ajouter un titre par lien » d'un event accepte **Spotify, Deezer et SoundCloud** (usage : edits/bootlegs introuvables en boutique). SoundCloud n'existe **qu'à ce point d'entrée** : les sources de bibliothèque restent **Spotify-only** (§4), l'acquisition library/collection reste **Deezer-only** (§6.5). L'ajout Deezer/SoundCloud exige le **module download activé** (opt-in §6.5) ; l'UI l'indique quand il est OFF. Le lien collé affiche une **pastille provider** immédiate, puis passe par un état transitoire `resolving` (UI seulement — le statut métier résultant reste ∈ §4 : `matched`/`ambiguous`/`missing`).

**COMMENT (build, POC-gated — la contrainte §2 légèreté tient).** SoundCloud sert du HLS → décodage **ffmpeg requis** (+40-80 Mo/plateforme). Décision build entre : **ffmpeg système optionnel détecté** (feature dégradée si absent, message clair) ou **composant téléchargeable séparé** (même mécanique que le module GPL-3, §6.5) — **jamais embarqué dans l'artefact de base**. Si le POC montre un coût/fragilité bloquant : SoundCloud **dégradé v1.1 sans bloquer la release** (faithful reporting), le champ n'acceptant alors que Spotify/Deezer.

### 11.2 Cycle de vie d'event — « modifié → ré-appliquer »

Un event `applied`/`partially_applied` **reste ouvert aux ajouts** (lien §11.1 ou recherche Deezer). Invariants :
...
- **Onboarding bi-phase 11 étapes** (Configuration 5 : welcome/Spotify/dossiers/module/scan + Prise en main 6 : modèle MyTags/bibliothèque/events/acquisition/santé/apply), rail cliquable, skippable, re-jouable — remplace le flux 4 étapes initial de SPEC-DESIGN §2. Sélecteur de langue présent dès l'onboarding (en plus de Réglages).
- **Modale d'arbitrage des acquisitions ambiguës** (candidats scorés, flag « match ISRC », méta durée/format, option « aucun de ces candidats ») = surface UI officielle de l'état `acquisition_ambiguous` (§5.5/D14).
...
- **Pastille provider sur SourceCard : retirée de la promesse** (les sources sont Spotify-only §4 — YAGNI ; les mocks `provider:'deezer'` sont une erreur de données de démo, correctif design). La pastille provider ne vit **que** sur l'ajout par lien (§11.1), où le multi-provider est réel.
