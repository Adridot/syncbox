export const en = {
  providers: {
    spotify: 'Spotify',
    deezer: 'Deezer',
    youtube: 'YouTube',
    soundcloud: 'SoundCloud',
  },
  linkImport: {
    title: 'Add a music link',
    placeholder: 'Paste a Spotify, Deezer, YouTube or SoundCloud link — track, album or playlist…',
    preview: 'Preview',
    busy: 'Resolving…',
    open: 'Open',
    chooseScope: 'Import this video or its playlist?',
    video: 'This video',
    playlist: 'The playlist',
    resolving: 'Resolving the linked music…',
    entries: '{n} entry | {n} entries',
    committed: '{n} track added to the event. | {n} tracks added to the event.',
    counts: '{selected} / {total} selected',
    unavailableCount: '{n} unavailable',
    selectAll: 'Select all available',
    selectNone: 'Select none',
    readd: 'Allow re-adding removed or ignored tracks',
    unavailable: 'Unavailable',
    repeated: 'Repeated',
    removed: 'Removed or ignored',
    alreadyPresent: 'Already in this event',
    noDownload: 'Confirmation adds the selected tracks. Download audio separately when needed.',
    confirm: 'Add {n} track | Add {n} tracks',
    retry: 'Retry preview',
    dismiss: 'Dismiss preview',
    setup: 'Open Settings',
    recent: 'Recent imports',
    states: {
      queued: 'Queued',
      resolving: 'Resolving',
      ready: 'Ready',
      failed: 'Failed',
      committed: 'Added',
      dismissed: 'Dismissed',
    },
    acquireSource: 'Download from {provider}',
    webTitle: 'Optional YouTube / SoundCloud acquisition',
    webSub:
      'Disabled by default. When enabled, Syncbox installs the pinned yt-dlp component separately (with its FFmpeg and JavaScript runtime) and uses it only for tracks imported from those links. Public content access depends on the provider.',
    webEnable: 'Enable web audio acquisition',
    webComponentLabel: 'yt-dlp component',
    webComponentInstalled: 'Installed at the pinned revision.',
    webComponentMissing: 'Not installed. Install it after enabling web audio acquisition.',
    webComponentUnavailable: 'No component release is bundled with this application build yet.',
    webComponentUnsupported: 'Only available on Apple silicon Macs.',
    webInstall: 'Install component',
    webInstalled: 'Web-audio component installed.',
    sourceQuality: 'Source: {codec} · {bitrate}',
    outputQuality: 'Output: {codec} · {rate}',
    transcoded: 'Converted to a supported format; source quality is unchanged.',
    errors: {
      source_identity_mismatch: 'The provider returned a different recording. The linked item was not replaced.',
      exact_deezer_item_unavailable: 'The exact linked Deezer item is unavailable.',
      job_source_intent_changed: 'The linked source changed. Start a new download for this track.',
      published_source_identity_mismatch: 'The saved download does not match this linked recording.',
      operation_timeout: 'The provider took too long to respond. Retry later.',
      incomplete_audio_output: 'The downloaded audio is incomplete. Retry this track.',
      web_audio_component_missing: 'Install the web-audio component in Settings.',
      web_audio_component_disabled: 'Enable web audio in Settings.',
      web_audio_release_unavailable: 'No web-audio component release is bundled with this application build yet.',
      unsupported_platform: 'Web audio acquisition is only available on Apple silicon Macs.',
      deezer_component_upgrade_required: 'Update the Deezer component to download the exact linked item.',
      provider_metadata_unavailable: 'The source could not return metadata. Retry later.',
      provider_rate_limited: 'The provider is limiting requests. Retry later.',
      provider_item_unavailable: 'This item is unavailable from its source.',
      provider_authentication_required: 'The provider requires access that this component does not support.',
      spotify_authentication_required: 'Connect Spotify in Settings to import albums or playlists.',
      spotify_collection_inaccessible: 'This Spotify collection is inaccessible to the connected account.',
      collection_items_inaccessible: 'The provider did not expose this collection’s tracks.',
      collection_empty_or_inaccessible: 'The provider returned no accessible collection contents.',
      unsupported_music_resource: 'Use a supported track, album or playlist link.',
      invalid_music_link: 'Enter a valid HTTPS music link.',
      collection_item_limit: 'This collection exceeds the 1,000-entry limit.',
      collection_incomplete: 'The provider returned fewer entries than it reports for this collection. Retry later.',
      metadata_time_limit: 'Metadata resolution exceeded its time limit. Try a smaller collection.',
      operation_cancelled: 'The operation was cancelled.',
      live_stream_unsupported: 'Live and upcoming streams cannot be imported.',
      provider_access_denied: 'The provider denied access to this item.',
      share_link_unavailable: 'This share link is unavailable. Try its full provider URL.',
      unsupported_network_destination: 'The link redirects to an unsupported destination.',
      private_network_destination: 'The provider link resolved to a non-public destination.',
      unsupported_share_target: 'This share link does not lead to a supported track, album or playlist.',
      invalid_video_id: 'The YouTube video identifier in this link is invalid.',
      invalid_playlist_id: 'The YouTube playlist identifier in this link is invalid.',
      collection_page_limit: 'This collection needs more pages than Syncbox will fetch.',
      metadata_byte_limit: 'The provider returned more metadata than Syncbox will store.',
      manifest_size_limit: 'This collection is too large to store as a preview.',
      invalid_collection_page: 'The provider returned an unreadable collection page.',
      invalid_manifest_source: 'The resolved collection does not match the submitted link.',
      invalid_manifest_entry: 'The provider returned an unreadable collection entry.',
      invalid_manifest_identity: 'The provider returned an entry whose identity could not be verified.',
      provider_timeout: 'The provider did not answer in time. Retry later.',
      web_audio_component_failed: 'The web-audio component failed. Retry, or reinstall it in Settings.',
      pending_import_limit: 'Too many previews are resolving at once. Wait for them to finish.',
      request_identity_conflict: 'This preview request was already used for a different link. Paste the link again.',
      invalid_request_token: 'This preview request is invalid. Paste the link again.',
      event_unavailable: 'This event is unavailable or being deleted.',
      import_not_ready: 'This preview is not ready to confirm.',
      import_not_retryable: 'Only a failed preview can be retried.',
      committed_import_immutable: 'A confirmed import cannot be dismissed.',
      committed_selection_conflict: 'This import was already confirmed with a different selection. Start a new preview to add more tracks.',
      invalid_import_selection: 'Select at least one entry to confirm.',
      invalid_readd_selection: 'Removed entries can only be re-added when they are selected.',
      unknown_entry_key: 'The selection no longer matches this preview. Reload it.',
    },
  },
  app: {
    title: 'Syncbox',
  },
  nav: {
    dashboard: 'Overview',
    library: 'Library',
    events: 'Events',
    history: 'Performances',
    health: 'Collection health',
    missing: 'Missing tracks',
    settings: 'Settings',
  },
  chrome: {
    systemState: 'System state',
    spotify: 'Spotify',
    rekordbox: 'Rekordbox',
    localJobs: 'Local jobs',
    connected: 'Connected',
    notConnected: 'Not connected',
    ready: 'Ready',
    rbOpenShort: 'Open',
    jobsActive: 'Running',
    jobsIdle: 'Idle',
  },
  rbGuard: {
    title: 'Rekordbox is open',
    body: 'Close Rekordbox so Syncbox can safely modify your collection. Review and diagnostics stay available.',
    confirm: 'I closed Rekordbox',
    blocked: 'Rekordbox open — blocked',
  },
  backendDown: {
    title: 'The Syncbox engine is not responding',
    body: 'Several restart attempts failed. Your collection was not touched — no write was in progress.',
    portCollision:
      'Port 8766 is used by another application. Close that application, then restart the engine. Syncbox did not stop it.',
    retry: '↻ Restart the engine',
    retrying: 'Restarting…',
  },
  status: {
    new: 'New',
    matched: 'Matched',
    conflict: 'Conflict',
    ambiguous: 'Ambiguous',
    ready: 'Ready',
    imported: 'Imported',
    missing: 'Missing',
    acquisition_failed: 'Acquisition failed',
    removed_from_source: 'Removed from source',
    removed_upstream: 'Gone from the playlist',
    ignored: 'Ignored',
    purchase_link_unavailable: 'Purchase link unavailable',
    manual_relink_needed: 'Manual relink needed',
    pending: 'Pending',
    applied: 'Applied',
    partially_applied: 'Partially applied',
  },
  quality: {
    ok: 'OK',
    lossy_source_probable: 'Lossy probable',
    incertain: 'Uncertain',
  },
  scope: {
    library: 'Library',
    event: 'Event',
    collection: 'Collection',
  },
  ownership: {
    label: 'Ownership',
    app_managed: 'Syncbox-managed',
    permanent_library: 'Permanent library',
    external: 'External',
  },
  common: {
    loading: 'Loading…',
    retry: 'Retry',
    cancel: 'Cancel',
    close: 'Close',
    networkError: 'The Syncbox engine is unreachable. Retry in a moment.',
  },
  spotify: {
    open: 'Open in Spotify',
    openFailed: 'Spotify could not be opened',
  },
  library: {
    tagline: 'Spotify playlists followed continuously (read-only).',
    syncAll: 'Sync all',
    syncOne: 'Sync',
    unfollow: 'Unfollow',
    unfollowConfirm: 'Confirm unfollow?',
    loadErrorTitle: 'Library unavailable',
    emptyTitle: 'No followed source',
    emptyBody: 'Add a Spotify playlist to keep it in sync with your Rekordbox collection.',
    addSource: '+ Add a Spotify source',
    sourcesTitle: 'Followed sources',
    searchPlaceholder: 'Filter playlists…',
    allSources: 'All sources',
    toReview: '{n} to review',
    tracksFollowed: '{n} tracks followed',
    allSub: '{playlists} playlists · {tracks} tracks followed',
    syncPartial: '{n} source(s) failed to sync',
    syncDone: '{n} source(s) synchronized',
    applyDone: '{n} track(s) applied to Rekordbox',
    restoredTo: 'restored → {status}',
    tableEmptyTitle: 'Nothing to review here',
    tableEmptyBody: 'No track matches this filter in this source.',
    filters: {
      review: 'To review',
      all: 'All',
      matched: 'Matched',
      conflict: 'Conflict',
      ready: 'Ready',
      missing: 'Missing',
      ignored: 'Ignored',
      removed_from_source: 'Removed',
    },
    selection: {
      selected: 'selected',
      editTags: 'Edit tags',
      apply: 'Apply to Rekordbox · {n}',
      hint: 'Check tracks to tag them in bulk',
      all: 'Select all visible tracks',
    },
    columns: {
      title: 'Title · confidence · bitrate',
      status: 'Status',
    },
    actions: {
      resolve: 'Resolve in the Missing center',
      restore: 'Restore',
      rematch: 'Re-match',
      ignore: 'Ignore',
    },
    add: {
      title: 'Add a source',
      sub: 'Paste a Spotify playlist link or pick one from your library. Syncbox follows it continuously (read-only).',
      modeLink: 'Paste a link',
      modePicker: 'My Spotify library',
      linkPlaceholder: 'https://open.spotify.com/playlist/…',
      resolving: 'Resolving…',
      pickerSearch: 'Search your playlists…',
      pickerEmpty: 'No playlist matches.',
      notConnected: 'Connect your Spotify account to continue.',
      privatePlaylist:
        'Spotify refused access to this playlist (404). Reconnect your account to re-authorize reading, then retry.',
      invalidLink: 'Paste a Spotify playlist link (or its 22-character id).',
      connectCta: 'Connect my Spotify account',
      resolved: 'resolved',
      tracksUnit: '{n} tracks',
      defaultTags: 'Default MyTags',
      follow: 'Follow this playlist',
      following: 'Following…',
    },
    rematch: {
      title: 'Re-match manually',
      sub: 'Spotify: {track}. Pick the Rekordbox counterpart.',
      noCandidates: 'No candidate found in the Rekordbox snapshot.',
      markMissing: 'Mark as missing',
      confirm: 'Confirm the match',
    },
  },
  events: {
    tagline:
      'Temporary imports — wedding, party. Applied as a smart playlist under “Event Imports”.',
    loadErrorTitle: 'Events unavailable',
    emptyTitle: 'No event',
    emptyBody: 'Create an event from a Spotify playlist or from scratch.',
    manualSource: 'Manual',
    appliedShort: 'applied {date}',
    pendingShort: 'pending',
    readyUnit: 'to apply',
    appliedUnit: 'applied',
    missingUnit: 'missing',
    pendingUnit: 'pending',
    titlesUnit: 'titles',
    claimDone: 'no staged file claimed | {n} staged file claimed | {n} staged files claimed',
    delete: 'Delete',
    rename: 'Rename',
    renameSave: 'Save',
    meta: '{n} titles · category {category}',
    appliedLine: 'Applied {date}',
    neverApplied: 'Never applied — in preparation',
    applyCta: 'Apply → Event Imports',
    reapplyCta: 'Re-apply · +{n}',
    addedChip: 'Added',
    resolveMissing: 'Resolve →',
    removeTrack: 'Remove this title',
    // §5.7 adoption — a file dropped in the event folder became a track
    adoptedChip: 'Dropped',
    adoptedChipHelp: 'Created from a file you dropped in the event folder.',
    duplicatesCollection:
      'Already in your collection — the existing title will be tagged, the dropped file was not needed.',
    // named when the collection snapshot could be read ({entry} = "title —
    // artist", or the title alone when the artist is missing)
    duplicatesCollectionNamed:
      'Already in your collection: {entry}. The existing title will be tagged, the dropped file was not needed.',
    rejectTrack: 'Ignore this dropped file',
    restoreTrack: 'Put back in the event',
    restoreDone: '“{title}” put back in the event.',
    // refresh from the Spotify playlist — no Rekordbox guard: nothing is
    // written to master.db
    refresh: 'Refresh from Spotify',
    refreshing: 'Refreshing…',
    refreshHelp:
      'Re-reads the Spotify playlist: imports the titles added to it, flags the ones that left. Writes nothing to Rekordbox.',
    refreshDone: '{added} added · {updated} updated · {removed} gone from the playlist.',
    refreshNoChange: 'The Spotify playlist has not moved — nothing to update.',
    // departure signal: a title is no longer in the playlist. Nothing was
    // written nor deleted — it is a decision to make, not pending work, so
    // it is never mixed into the “+n” badge
    removedShort: 'gone',
    removedHelp:
      'These titles are no longer in the Spotify playlist. Nothing was touched in Rekordbox: keep them or remove them, your call.',
    keepTrack: 'Keep this title',
    keepDone: '“{title}” kept — the departure signal is cleared.',
    filterEmpty: 'No title in this filter.',
    modifiedBanner:
      'Modified since the last apply — {n}. Re-apply to update the Rekordbox smart playlist.',
    changesPending: '{n} change pending | {n} changes pending',
    claim: 'Claim staged files',
    openStaging: 'Open the drop folder',
    openStagingHelp: 'Drop purchased or hand-retrieved files here, then “Claim staged files”.',
    downloadMissing: 'Download the missing ({n})',
    filters: {
      all: 'All',
      ready: 'Ready',
      missing: 'Missing',
      ambiguous: 'To arbitrate',
      pending: 'Pending',
      ignored: 'Rejected',
      removed: 'Gone',
    },
    columns: {
      title: 'Title',
      status: 'Staging status',
      conf: 'Conf.',
      action: 'Action',
    },
    new: {
      title: 'New event',
      sub: 'Import a Spotify playlist, or create an empty event and add titles by hand.',
      cta: '+ New event',
      modeFollowed: 'Followed playlist',
      modePlaylist: 'From a link',
      modeEmpty: 'Empty',
      followedLabel: 'Followed playlist',
      followedPick: 'Choose a playlist…',
      followedEmpty: 'No followed playlists yet — add one in Library, or paste a link.',
      nameLabel: 'Event name',
      namePlaceholder: 'New Year’s Eve party',
      nameHelp: 'Also used as its MyTag · category {category}',
      linkLabel: 'Playlist link',
      linkPlaceholder: 'Spotify playlist URL…',
      linkHelp: 'All the playlist’s titles are imported at once.',
      creating: 'Creating…',
      createPlaylist: 'Import the playlist',
      createEmpty: 'Create the empty event',
    },
    apply: {
      title: 'Apply “{name}”',
      lead: 'Creates a smart playlist under {parent} and tags the ready tracks. Write preview:',
      playlistLine: 'Smart playlist created',
      taggedLine: 'Tracks tagged “{tag}”',
      missingLine: 'Missing tracks (skipped)',
      backupNote: 'Timestamped backup before the write. Reversible.',
      confirm: 'Confirm & apply',
    },
    reapply: {
      title: 'Re-apply “{name}”',
      lead: 'Updates the existing smart playlist with the pending changes. Only the {delta} is written:',
      deltaWord: 'delta',
      playlistLine: 'Smart playlist updated',
      readyLine: 'Added & ready titles (tagged)',
      missingLine: 'Added & missing titles',
      note: 'New timestamped backup before the write. Already-applied titles are untouched.',
      confirm: 'Re-apply · {n} change | Re-apply · {n} changes',
    },
    del: {
      title: 'Delete “{name}”',
      lead: 'Summary of what will happen when this event is deleted:',
      tagLine: 'Event MyTag removed',
      playlistLine: 'Rekordbox playlists removed',
      filesCount: '{n} file | {n} files',
      tracksCount: '{n} track planned | {n} tracks planned',
      noTracks: 'No Rekordbox track is linked to this event.',
      contentId: 'Content ID',
      sourcePath: 'Source path',
      retainingTags: 'Other active MyTags',
      noRetainingTags: 'None',
      destinationPath: 'Migration destination',
      anlz: 'ANLZ',
      anlzRequired: 'Path update required (backed up before writing)',
      anlzUnchanged: 'No path update',
      xmlArtifacts: 'XML artifacts',
      stagingArtifacts: 'Staging artifacts',
      expectedDeletions: 'Expected file deletions',
      action: {
        already_permanent: 'Keep in place',
        migrate_to_collection: 'Migrate to Collection',
        delete_with_event: 'Delete with event',
        keep_in_place: 'Keep in place',
      },
      actionHelp: {
        already_permanent: 'Permanent Collection files stay unchanged.',
        migrate_to_collection: 'Event-owned files with other tags move to Collection.',
        delete_with_event: 'Event-owned files with no other tag are removed with the event.',
        keep_in_place: 'Pre-existing files only lose their event membership.',
      },
      unresolved: '{n} unresolved case | {n} unresolved cases',
      issue: {
        active_acquisition:
          'A download is still queued or running. Wait for it to finish, then refresh this preview.',
        missing_retained_source:
          'This retained track has no file on disk. Relink it or remove it in Rekordbox, then refresh this preview.',
        unsafe_retained_source:
          'This retained track does not point at a regular file. Relink it or remove it in Rekordbox, then refresh this preview.',
      },
      technicalDetails: 'Technical details',
      resolveFirst: 'Resolve the remaining cases',
      confirmCount: 'Delete event · {n} file | Delete event · {n} files',
      note: 'Required migrations complete before event cleanup. Rekordbox identity and retained metadata stay unchanged. Requires Rekordbox closed.',
      confirm: 'Delete the event',
    },
    // add-event-track-removal: the only destructive move in the workspace —
    // every label must say exactly what is lost.
    remove: {
      title: 'Remove tracks from “{name}”',
      lead: 'What happens, track by track. Nothing is written until you confirm.',
      untagLine: 'Tracks removed from the event',
      rbDeleteLine: 'Entries deleted from Rekordbox',
      trashLine: 'Files sent to the trash',
      tracksCount: '{n} track in the batch | {n} tracks in the batch',
      noTracks: 'No track selected.',
      trashFlag: 'file to the trash',
      action: {
        already_permanent: 'Untagged · kept',
        keep_in_place: 'Untagged · kept',
        delete_with_event: 'Deleted from Rekordbox · file to the trash',
        never_applied: 'Never applied · staged file to the trash',
        shared_with_kept_track: 'Removed from the event only',
      },
      actionHelp: {
        already_permanent:
          'The event tag is removed. The track stays in Rekordbox and its file is untouched.',
        keep_in_place:
          'The event tag is removed. The track stays in Rekordbox and its file is untouched.',
        delete_with_event:
          'The event tag is removed, the Rekordbox entry is deleted, and the file goes to the trash. This is the only destructive outcome in this batch.',
        never_applied:
          'The track leaves the event and its staged file goes to the trash. Rekordbox is not touched.',
        shared_with_kept_track:
          'A track staying in the event shares its Rekordbox entry or its file. Nothing is untagged, nothing is deleted: the row leaves the event, and that is all.',
      },
      unresolved: '{n} unresolved case | {n} unresolved cases',
      issue: {
        retained_by_other_mytag:
          'This track’s staged file also carries a MyTag you added in Rekordbox ({tags}). While it remains, the batch cannot run.',
      },
      option: {
        remove_other_mytag: 'Remove that other MyTag in Rekordbox, then reopen this preview.',
        delete_event:
          'Or delete the whole event — event deletion migrates the file into Collection instead of losing it.',
        relink_in_rekordbox: 'Relink the file in Rekordbox, then reopen this preview.',
        remove_from_rekordbox: 'Remove the entry in Rekordbox, then reopen this preview.',
      },
      technicalDetails: 'Technical details',
      resolveFirst: 'Resolve the remaining cases',
      confirmCount: 'Remove {n} track | Remove {n} tracks',
      filesCount: '{n} file to the trash | {n} files to the trash',
      note: 'Timestamped Rekordbox backup before the write. The event, its MyTag and its playlist survive: only the batch leaves. Requires Rekordbox closed.',
      noteNoRb:
        'None of these tracks was ever applied: nothing is written to Rekordbox, it can stay open.',
      selectionCta: 'Remove from the event · {n}',
      selectAll: 'Select all',
      done: '{n} track removed from the event | {n} tracks removed from the event',
    },
  },
  history: {
    tagline: 'Every gig rebuilt from the Rekordbox play history — crash-proof, read-only.',
    emptyTitle: 'No performance yet',
    emptyBody:
      'Play something in Rekordbox and it will appear here. Everything already in your Rekordbox history is imported automatically.',
    loadErrorTitle: 'Could not load the performance history',
    liveBanner: 'Performance in progress — {n} tracks played, last one at {time}',
    liveShow: 'Open the live tracklist',
    filters: {
      main: 'Main sets',
      all: 'Everything',
      hidden: 'Hidden',
    },
    unnamed: 'Performance of {date}',
    tracksUnit: 'tracks',
    sessionsUnit: '{n} Rekordbox session | {n} Rekordbox sessions',
    cutBadge: 'cut',
    cutRow: 'Rekordbox stopped {ended} — resumed {resumed}',
    usbBadge: 'USB import',
    usbNote:
      'Imported from a USB/CDJ history: track order is real, clock times are the import moment.',
    overlapBadge: 'parallel machine',
    overlapNote:
      'Another computer on this Rekordbox account was playing during the same time window.',
    rename: 'Rename',
    renameSave: 'Save',
    hide: 'Hide',
    unhide: 'Restore',
    exportCta: 'Create Rekordbox playlist',
    exportDone: 'Playlist “{name}” created in the “{folder}” folder — {n} tracks.',
    exportSpotify:
      '{n} Spotify track was restored into the collection and included. | {n} Spotify tracks were restored into the collection and included.',
    exportRecovered:
      '{n} locally-deleted track was recovered as a Spotify streaming reference. | {n} locally-deleted tracks were recovered as Spotify streaming references.',
    exportSkipped:
      '{n} track no longer in the collection was skipped. | {n} tracks no longer in the collection were skipped.',
    spotifyPending: 'Spotify track (title pending)',
    columns: {
      time: 'Time',
      title: 'Title',
    },
  },
  health: {
    tagline: 'Diagnostics, cleanup and backups — always previewed before writing.',
    tabs: {
      duplicates: 'Duplicates',
      missing: 'Missing files',
      untagged: 'Untagged',
      smartfixes: 'Smart Fixes',
      backups: 'Backups & logs',
    },
  },
  duplicates: {
    intro:
      'Pick the track to {keep} in each group; the others will be removed. The suggestion compares file availability, quality and bitrate, then uses a stable tie-break. Ownership is informational only. You can change the keeper.',
    keepWord: 'keep',
    scanCta: 'Scan for duplicates',
    scanning: 'Scanning…',
    scanJob: 'Duplicates scan',
    scanDone: 'no duplicate group found | {n} duplicate group found | {n} duplicate groups found',
    resolved: 'Group resolved — {n} row(s) soft-deleted, {files} file(s) processed.',
    rescanHint: 'The collection changed: re-run the scan before resolving another group.',
    stale: 'The collection changed since this scan — re-run it before resolving.',
    rescan: 'Re-run the scan',
    notScannedTitle: 'No scan yet',
    notScannedBody:
      'The spectral scan reads every group member and locks the engine while it runs — launch it explicitly.',
    cleanTitle: 'No duplicates',
    cleanBody: '{n} tracks scanned — nothing to merge.',
    perGroupNote: 'Confirmation {perGroup} — no automatic bulk resolution.',
    perGroupWord: 'per group',
    group: 'Group {n}',
    method: { isrc: 'ISRC', fuzzy: 'Fuzzy' },
    reason: {
      file_present: 'kept: file present on disk',
      quality: 'kept: best quality (D6 scale)',
      date: 'kept: oldest import (tie-break)',
      content_id: 'kept: stable content ID tie-break',
      identical: 'members are equivalent — pick freely',
    },
    titlesDiverge: 'titles diverge — check before resolving',
    keepTag: 'Keep',
    removeTag: 'Remove',
    outcome: 'keep “{keep}” · remove {n}',
    notADuplicate: 'Not a duplicate',
    resolveGroup: 'Resolve this group',
    attrQuality: 'Quality',
    attrFile: 'File',
    attrPlaylists: 'Playlists',
    attrCues: 'Cues',
    fileMissing: 'missing',
    filePresent: 'present',
    qualityReason: {
      sharp_cutoff_in_lossless_container:
        'sharp spectral cutoff in a lossless container (fake-FLAC probable)',
      cutoff_indicates_le192_source: 'spectral cutoff indicates a ≤192 kbps source',
      cutoff_in_256_class_zone: 'cutoff in the 256 kbps zone — uncertain',
      lossless_container_cutoff_in_320_v0_zone: 'cutoff at the 320/V0 border — uncertain',
      spectral_cutoff_ambiguous:
        'spectral cutoff is ambiguous — possible band-limited master or lossy source',
    },
  },
  missing: {
    collectionIntro:
      'Collection rows whose audio file is gone on disk. Purchase links open in your browser — the app never contacts stores.',
    openCenter: 'Open the Missing tracks center',
    loadErrorTitle: 'Missing files unavailable',
    ignored: '“{title}” ignored.',
    removed: '“{title}” removed from the collection (soft-delete, reversible).',
    relinked: '“{title}” relinked.',
    acquired: '“{title}” downloaded to the Syncbox acquisition folder.',
    acquisitionFailed: '“{title}” could not be downloaded. Purchase links remain available.',
    purchaseOpenFailed: 'The purchase link could not be opened in your browser. Try again.',
    undo: 'Undo',
    untitled: '(untitled)',
    buyOn: 'Buy on {store} ↗',
    buyMenu: 'Buy ({n} stores) ↗',
    acquireDeezer: 'Download with Deezer',
    relinkCta: 'Relink a file',
    ignoreCta: 'Ignore',
    removeCta: 'Remove',
    removeTitle: 'Soft-delete the Rekordbox row (reversible; the audio is never touched)',
    selectAll: 'Select all',
    moreActions: 'More actions',
    bulkIgnore: 'Ignore · {n}',
    bulkAcquire: 'Download with Deezer · {n}',
    bulkRemove: 'Remove · {n}',
    bulkIgnored: '{n} track(s) ignored.',
    bulkRemoved: '{n} row(s) removed from the collection (soft-delete, reversible).',
    bulkAcquireDone: '{ok} downloaded · {failed} failed. Purchase links remain available.',
    acq_queued: 'Queued',
    acq_running: 'Downloading…',
    acq_downloaded: 'Downloaded',
    acq_downgraded: 'Downloaded · reduced quality',
    acq_failed: 'Failed',
    acqProgress: 'Downloads: {done}/{total}',
    searchDeezer: 'Search on Deezer',
    errors: {
      notStreamable: 'Not streamable on Deezer — try the manual search, another edition often is.',
      notFullTrack: 'The downloaded file is not the full track — try the manual search.',
      isrcNotFound: 'Not found on Deezer by ISRC — use the manual search.',
      rekordboxOpen: 'Rekordbox is open — close it, then retry.',
    },
    emptyTitle: 'Nothing missing here',
    emptyBody: 'Every track in this scope has a resolved file.',
  },
  deezer: {
    panelTitle: 'Deezer search',
    trackContext: 'For: {track}',
    placeholder: 'Artist title…',
    searchCta: 'Search',
    searching: 'Searching…',
    empty: 'No results.',
    download: 'Download',
    play: 'Play the 30 s preview',
    pause: 'Pause the preview',
  },
  missingCenter: {
    tagline: 'Recover missing tracks — library, events and collection, in one place.',
    recommended: 'RECOMMENDED',
    legalTitle: 'Buy the missing tracks',
    legalBody:
      'The default path: support the artists and get the best quality. Syncbox opens a deep search in your browser — no data goes through the app. Already lawfully own the file? Relink it manually.',
    allScopes: 'All scopes',
    loadErrorTitle: 'Missing tracks unavailable',
  },
  relink: {
    title: 'Relink to a local file',
    sub: '{track} — pick the local file you already lawfully own.',
    noCandidates:
      'No local candidate found (inbox and known folders scanned). Drop the file into the Syncbox inbox and retry.',
    none: 'None of these candidates',
    linking: 'Relinking…',
    confirm: 'Relink this file',
  },
  untagged: {
    intro:
      'Priority order: {junk} < {dup} < {alt} < {review}. Removal is a reversible Rekordbox soft-delete; audio is never touched. Ownership is informational.',
    cat: {
      all: 'All',
      junk: 'junk',
      dup_of_tagged: 'duplicate',
      alt_version: 'alt version',
      review: 'to review',
    },
    loadErrorTitle: 'Untagged unavailable',
    selectAll: 'Select all',
    deleteSelection: 'Delete selection',
    deleted: 'nothing deleted | {n} row soft-deleted | {n} rows soft-deleted',
    skipped: '{n} skipped | {n} skipped',
    skipReason: {
      tagged: 'now tagged',
      not_found: 'not found',
    },
    empty: 'No untagged track in this category.',
    patterns: {
      title: 'Junk patterns ({n})',
      help: 'User regex patterns flagging junk titles, on top of the universal structural rules. An invalid regex is rejected by the engine.',
      placeholder: 'e.g. ^www\\..*',
      add: 'Add',
    },
  },
  smartfixes: {
    title: 'Bulk metadata cleanup',
    lead: 'Conservative structural fixes from a fixed catalog. The cycle is always {cycle}: nothing is written without your confirmation.',
    cycle: 'preview → confirm → write',
    families: {
      cleanup:
        'Clean trailing site URLs, orphan separators and Unicode whitespace; normalize Unicode to NFC',
      encoding:
        'Repair selected common reversible UTF-8 / Latin-1 / Windows-1252 mojibake signatures and the exact named XML entities &amp; &quot; &apos; &lt; &gt;',
      credits: 'Extract only unambiguous terminal featured-artist and remixer credits',
    },
    limitsNote:
      'Credits stay conservative: a terminal featured artist is added to the existing artist credit; an empty remixer is filled only from a terminal “(Name Remix)”, while the remix title is preserved. Stylized uppercase and mixed-case names stay unchanged; ambiguous patterns are skipped.',
    backupNote:
      'The read-only preview can run while Rekordbox is open. Writing requires it closed, changes collection metadata regardless of file ownership, touches no audio file, and first creates a timestamped database backup.',
    dryRunCta: 'Run the preview (dry-run)',
    executed: '{fields} fields written on {tracks} tracks. Timestamped backup created.',
    fields: {
      title: 'Title',
      artist: 'Artist',
      remixer: 'Remixer',
    },
    dryrun: {
      title: 'Fix preview',
      lead: 'Nothing has been written. This is the exact effect, field by field, before → after. Confirming applies {exactly} these changes.',
      exactly: 'exactly',
      nothing: 'Nothing to fix — the collection is already clean.',
      notSet: 'Not set',
      emptyValue: 'Empty value',
      trackId: 'Track ID {id}',
      wsLegend:
        'red dots mark invisible whitespace (leading/trailing, doubled, non-breaking) — the change is real even when both sides look identical',
      stale:
        'The collection changed since this preview — re-run the dry-run to see the exact effect before writing.',
      rerun: 'Re-run the preview',
      summary: 'no field changes | {n} field change | {n} field changes',
      confirm: 'Write {n} change | Write {n} changes',
    },
  },
  backups: {
    title: 'Syncbox restore points',
    rotation: 'rotation:',
    recent: 'recent:',
    save: 'Save',
    restore: 'Restore',
    restored: 'Restored {name} — the previous DB was snapshotted first ({snapshot}).',
    retentionSaved: 'Recent tier set to {n} restore points.',
    policy:
      'Keeps the {n} most recent, then 48 hourly, 30 daily, 12 weekly and 12 monthly restore points.',
    verified: 'verified',
    legacy: 'legacy backup',
    coherent: 'Rekordbox + Syncbox',
    rekordboxOnly: 'Rekordbox only — coherent restore unavailable',
    pinned: 'pinned',
    reason: {
      rekordbox_mutation: 'Rekordbox change',
      event_apply: 'Event apply',
      event_reapply: 'Event re-apply',
      event_delete: 'Event deletion',
      library_apply: 'Library apply',
      collection_relink: 'Collection relink',
      missing_remove: 'Missing entry removal',
      duplicate_resolve: 'Duplicate resolution',
      untagged_remove: 'Untagged cleanup',
      smart_fixes: 'Smart fixes',
      acquisition_storage_migration: 'Download storage migration',
      legacy_metadata_backfill: 'Legacy metadata repair',
      pre_restore: 'Before restore',
      legacy: 'Unknown operation',
    },
    migration: {
      title: 'Legacy download storage',
      lead: 'Moves verified files to their permanent owner and cleans their complete legacy job folders.',
      confirm: 'Process {n} item | Process {n} items',
      doneFiles: '{n} legacy file migrated | {n} legacy files migrated',
      doneDirectories: '{n} job directory cleaned | {n} job directories cleaned',
      event: '{n} event file | {n} event files',
      collection: '{n} Collection file | {n} Collection files',
      cleanup: '{n} job directory to clean | {n} job directories to clean',
      ignored: '{n} item left untouched | {n} items left untouched',
      readyDetails: 'Files ready to migrate',
      cleanupDetails: 'Complete job directories ready to clean',
      ignoredDetails: 'Special cases left untouched',
      reason: {
        missing_source: 'The recorded source file is missing.',
        unsafe_source: 'The recorded source path is outside the managed acquisition area.',
        missing_owner: 'The original event or library track no longer exists.',
        unsafe_destination: 'The event or Collection destination is outside managed storage.',
        destination_collision: 'No safe destination filename could be allocated.',
        missing_destination: 'A previously copied destination is missing or no longer matches its source.',
        missing_rekordbox_content: 'The linked Rekordbox content no longer exists.',
        rekordbox_path_changed: 'The Rekordbox path changed after the download.',
        unsafe_analysis: 'A linked Rekordbox analysis file is outside its managed directory.',
        orphan_file: 'No acquisition job owns this file.',
        active_job: 'The acquisition job is still active.',
        unsafe_directory: 'The job directory is symbolic or outside managed storage.',
        unowned_directory: 'No acquisition job owns this directory.',
        unverified_destination: 'The published destination is not referenced by its owner.',
      },
    },
    empty: 'No backup yet — one is created before every write.',
    restoreNote:
      'Each new restore point contains Rekordbox and Syncbox state. Restore snapshots the current state first and requires Rekordbox closed.',
    logsTitle: 'Logs',
    logsUnconfigured: 'No log file configured on this engine.',
    logsEmpty: 'The log file is empty.',
    openLogsFolder: 'Open the logs folder',
  },
  settings: {
    replayOnboarding: 'Replay the onboarding',
    spotify: {
      connected: 'Connected · read-only · PKCE',
      notConnected: 'Not connected',
      connect: 'Connect',
      reconnect: 'Reconnect',
      connecting: 'Connecting…',
      disconnect: 'Disconnect and delete Spotify data',
      disconnectConfirm:
        'Disconnect Spotify? Syncbox will delete followed playlists and their local sync history, detach Spotify IDs from events, and preserve local Rekordbox data, audio files, and your Client ID.',
      disconnected: 'Spotify disconnected and local playlist relationships deleted.',
      privacy: 'Spotify data and privacy',
      timeout: 'Spotify authorization timed out. Retry and complete the browser approval.',
      authorizationFailed: 'Spotify authorization was declined or failed. Retry in the browser.',
      callbackPortInUse:
        'Spotify cannot connect because callback port 8765 is in use. Close the application holding that port, then retry. Syncbox did not stop it.',
      failed: 'connection failed — retry',
      needClientId: 'Enter your Spotify Client ID below to enable the connection.',
      clientIdLabel: 'Spotify Client ID',
      clientIdPlaceholder: '32-character hex id from your Spotify app',
      clientIdSaved: 'Spotify Client ID saved.',
      clientIdHelpTitle: 'Create my Spotify app, step by step',
      helpIntro:
        'Syncbox connects through your own Spotify Development Mode app, created once. The app owner must have an active Spotify Premium subscription. Development Mode supports at most 5 allowlisted users. Allow 5 minutes:',
      helpStep1:
        'Open the Spotify Developer Dashboard and sign in with your regular Spotify account.',
      helpStep2: '“Create app” — pick any name (e.g. Syncbox) and description.',
      helpStep3:
        'In the “Redirect URIs” field, paste EXACTLY this address (Spotify checks it character for character), then “Add”:',
      helpStep3warn:
        '“localhost” does not work: Spotify requires the 127.0.0.1 address, exactly as above.',
      helpStep4:
        'For “Which API/SDKs are you planning to use?”, tick “Web API”, accept the terms and Save.',
      helpStep5:
        'On your app page, add the account under User Management if it is not the owner, then “Settings” → copy the “Client ID” (NOT the secret — none is needed) into the field above, then Validate and Connect.',
      redirectErrorHint:
        'If Spotify shows “redirect_uri: Not matching configuration”, the address registered in your Spotify app does not match: go back to its Settings and re-paste the address above, with no extra space or trailing slash.',
      copy: 'Copy',
      copied: 'Copied ✓',
    },
    deezer: {
      title: 'Optional Deezer acquisition',
      sub: 'Disabled by default. When enabled, Syncbox installs the pinned streamrip component separately and uses it only for explicit missing-track downloads. Purchase links remain the recommended path.',
      enable: 'Enable Deezer acquisition',
      arlLabel: 'Premium Deezer ARL',
      arlPlaceholder: 'Paste ARL',
      arlStored: 'ARL stored',
      arlHelp:
        'Stored only in the encrypted local secrets store; never exported in settings or data files.',
      saveArl: 'Save ARL',
      deleteArl: 'Delete ARL',
      arlSaved: 'Deezer ARL saved.',
      arlDeleted: 'Deezer ARL deleted.',
      componentLabel: 'streamrip component',
      componentInstalled: 'Installed at the pinned revision.',
      componentMissing: 'Not installed. Install it after enabling acquisition.',
      install: 'Install component',
      installed: 'Deezer component installed.',
    },
    paths: {
      title: 'Folders',
      validate: 'Validate',
      browse: 'Browse…',
      dbLabel: 'Rekordbox database (master.db)',
      dbHelp:
        'The encrypted database where Rekordbox keeps your collection (tracks, playlists, cues, MyTags). Syncbox reads it and writes MyTags / smart playlists into it — with a timestamped backup before every write.',
      useMacDefault: 'Use the macOS default',
      dbFindTitle: 'How do I find it?',
      dbFindBody:
        'Rekordbox → Preferences → Advanced → Database shows the folder. On macOS it is almost always ~/Library/Pioneer/rekordbox/master.db.',
      rootLabel: 'Storage root',
      rootHelp:
        'The folder holding your music and the Syncbox working area. Files under storage_root/rekordbox/ belong to the permanent library; Syncbox-managed working files live under storage_root/_syncbox/. Safety depends on the operation and ownership class.',
      derivedTitle: 'Derived layout (read-only)',
      derived: {
        permanent_library: 'Permanent library (your audio)',
        permanent_libraryDesc:
          'Your permanent library, in subfolders of your choosing. The “rekordbox” name is fixed because it is part of the paths Rekordbox stores in master.db — changing it would break existing tracks.',
        inbox: 'Inbox (staged imports)',
        inboxDesc:
          'Drop manually retrieved audio files here: the Missing tracks center offers them for relinking automatically.',
        events: 'Events staging',
        eventsDesc:
          'Files prepared for each event, one subfolder per event, before they move into your collection.',
        backups: 'Database backups',
        backupsDesc:
          'A verified Rekordbox + Syncbox restore point before EVERY write, thinned by the retention policy below.',
      },
      derivedNote:
        'The _syncbox folders are created automatically on first use; their names are fixed because paths already written into master.db point at them. Your audio folders under rekordbox/ are yours — Syncbox only reads them.',
    },
    retention: {
      title: 'Backup retention',
      sub: 'Recent points kept before progressive hourly, daily, weekly and monthly thinning (0 = unlimited).',
    },
    language: {
      title: 'Language',
    },
    transfer: {
      title: 'Export & import',
      sub: 'Move your configuration to another machine, or keep a full snapshot of Syncbox’s own data. Neither file ever contains your Spotify tokens.',
      settingsLabel: 'Settings (JSON)',
      settingsDesc: 'Paths, Spotify Client ID, matching knobs — one small readable file.',
      dataLabel: 'All data (database)',
      dataDesc:
        'Settings plus sources, events and history in one database snapshot. Importing replaces the current data — a safety backup is made first.',
      export: 'Export…',
      import: 'Import…',
      exportedTo: 'Exported to {path}',
      imported: 'Settings imported.',
      importedPartial: 'Settings imported — skipped (not valid on this machine): {skipped}',
      dataImportConfirm:
        'Replace all Syncbox data with this file? The current data is snapshotted first (reversible).',
      dataImported: 'Data imported. The previous data was saved as {backup}.',
      dataImportedNoBackup: 'Data imported.',
      needShell: 'Available in the desktop app (native file dialogs).',
    },
    advanced: {
      title: 'Advanced · matching thresholds',
      warning:
        'The defaults are recommended and calibrated. Change them only if you know what you are doing — a per-track manual re-match is almost always better.',
      threshold: 'Confidence threshold',
      margin: 'Ambiguity margin',
      weights: 'Weights (sum = 1.00)',
      weight_title: 'Title',
      weight_artist: 'Artist',
      weight_duration: 'Duration',
      sumError: 'The three weights must sum to exactly 1.00.',
      isrcPolicy: 'ISRC collision policy',
      isrc_guarded: 'Reject if |Δduration| > 15 s AND title < 82 (default)',
      isrc_trust: 'Always trust the ISRC',
      isrc_strict: 'Strict — reject on any collision signal',
      lockedTitle: 'Locked — algorithm invariants',
      lockedBody:
        'Exact ISRC takes priority over fuzzy · single normalization pipeline (matching + dedup) · duration buckets. Not editable.',
      reset: 'Reset to defaults',
      save: 'Save',
      saved: 'Matching thresholds saved.',
      resetDone: 'Thresholds reset to the calibrated defaults.',
    },
  },
  onboarding: {
    skip: 'Skip the introduction',
    skipStep: 'Skip this step',
    back: 'Back',
    next: 'Next',
    scanning: 'Reading…',
    scanDone: '{n} tracks found in your Rekordbox collection.',
    phase: {
      setup: 'Configuration',
      tour: 'Getting started',
    },
    steps: {
      welcome: {
        rail: 'Welcome',
        tag: 'Welcome',
        title: 'Syncbox links Spotify to Rekordbox',
        sub: 'You keep your playlists on Spotify; Syncbox finds each title in your Rekordbox collection, flags what is missing, and prepares your sets — without ever writing to your Spotify account.',
        cta: 'Set up Syncbox',
      },
      spotify: {
        rail: 'Spotify',
        tag: 'Configuration · 2 / 4',
        title: 'Connect your Spotify account',
        sub: 'Read-only, via OAuth PKCE with your own Client ID. Syncbox reads your followed playlists — it never writes anything to your account.',
        cta: 'Connect Spotify',
      },
      folders: {
        rail: 'Folders',
        tag: 'Configuration · 3 / 4',
        title: 'Point Syncbox at your folders',
        sub: 'The Rekordbox database (master.db) and your storage root. Each path is validated by the engine before you continue.',
        cta: 'Validate the paths',
      },
      scan: {
        rail: 'First scan',
        tag: 'Configuration · 4 / 4',
        title: 'First read of your collection',
        sub: 'Syncbox reads your Rekordbox database. Nothing is modified — it is a pure read.',
        cta: 'Read the collection',
      },
      model: {
        rail: 'The model',
        tag: 'Getting started · The principle',
        title: 'Your collection, organized by MyTags',
        sub: 'Syncbox assumes your Rekordbox library is organized with MyTags rather than frozen playlists. Each followed Spotify playlist becomes a MyTag: a title stays unique in your collection, and its tags say which playlists it belongs to.',
        cta: 'See the library',
      },
      library: {
        rail: 'Library',
        tag: 'Getting started · Library',
        title: 'Every playlist matched against your collection',
        sub: 'For each followed source, Syncbox shows what is already matched in Rekordbox and what is missing. A title you already own is never duplicated — it simply receives the source’s MyTag.',
        cta: 'Next',
      },
      events: {
        rail: 'Events',
        tag: 'Getting started · Events',
        title: 'An event = a dated gig',
        sub: 'Example: a wedding on July 12. You add the couple’s Spotify playlist → Syncbox integrates the titles you already own (zero duplicates) and only lists the missing ones to recover.',
        cta: 'Next',
      },
      missing: {
        rail: 'Missing tracks',
        tag: 'Getting started · Missing tracks',
        title: 'Recover only what is missing',
        sub: 'Missing titles from a playlist or an event gather in one center. The legal purchase path (Beatport, Bandcamp) comes first; a file you already lawfully own can be relinked manually.',
        cta: 'Next',
      },
      health: {
        rail: 'Health',
        tag: 'Getting started · Health',
        title: 'Keep your collection clean',
        sub: 'Duplicates, missing files and untagged titles live in one place — always previewed before any write.',
        cta: 'Next',
      },
      apply: {
        rail: 'Apply',
        tag: 'Getting started · Safety',
        title: 'Apply an event as a smart playlist',
        sub: 'When an event is ready, Syncbox creates a Rekordbox smart playlist filtered on its MyTag. Automatic backup before every write, reversible deletions, and everything is blocked while Rekordbox is open.',
        cta: 'Start',
      },
    },
  },
  consent: {
    irreversible: {
      title: 'Irreversible file deletion',
      lead: 'This file lives on a {volume} volume with no trash. {lost} — it cannot be restored.',
      volume: 'cloud / exFAT',
      lost: 'The audio will be permanently lost',
      reversibleNote:
        'The Rekordbox row stays reversible (soft-delete + backup). Only the audio is final.',
      checkbox: 'I understand the audio will be permanently lost on this volume.',
      confirm: 'Delete permanently',
    },
    anlz: {
      title: 'Replace the file — cue points to check',
      lead: 'Relinking replaces the audio file associated with this Rekordbox track. Your {cues} are stored separately (ANLZ files) and {uncovered}: if the new file differs, they may shift and need a re-analysis.',
      cues: 'cue points, beatgrid and waveform',
      uncovered: 'are not covered by the backup',
      reversibleNote:
        'The Rekordbox database (tags, playlists, smart playlists) stays backed up and reversible. Only the replaced file’s cue points may need redoing.',
      checkbox: 'Understood — this file’s cue points may need redoing.',
      confirm: 'Continue · relink the file',
    },
  },
  tags: {
    bulkTitle: 'Bulk tag edit',
    bulkSub:
      'Tags apply as a delta: only the listed tags are added or removed. The tracks’ other MyTags are preserved.',
    addLabel: '+ Add',
    removeLabel: '− Remove',
    addEmpty: 'No tag to add — pick below.',
    removeEmpty: 'No tag to remove.',
    searchPlaceholder: 'Search among {n} MyTags…',
    colAdd: '+ add',
    colRemove: '− remove',
    create: 'Create “{name}”',
    catalogEmpty: 'No MyTag in the Rekordbox collection yet.',
    noMatch: 'No MyTag matches.',
    tracksUnit: 'tracks',
    applyDelta: 'Apply the delta',
    addPlaceholder: '+ tag',
  },
  dashboard: {
    tagline: 'Your Rekordbox collection, under control.',
    syncSources: 'Sync sources',
    createEvent: '+ Create an event',
    hero: {
      closedTitle: 'Rekordbox closed — mutations allowed',
      closedSub: 'Timestamped backup before every write. Deletions are reversible.',
      openTitle: 'Rekordbox is open — writes paused',
      openSub: 'Review, diagnostics and purchase links stay available. Close Rekordbox to apply.',
      lastBackup: 'Last backup',
      noBackup: 'none yet',
    },
    tiles: {
      sources: 'Followed sources',
      toReview: '{n} tracks to review',
      openLibrary: 'open the library',
      duplicates: 'Duplicates detected',
      groupsToConfirm: 'groups to confirm',
      runScan: 'run a scan',
      missing: 'Missing files',
      purchaseOrRelink: 'purchase or relink',
      untagged: 'Metadata to clean',
      smartFixes: 'Smart Fixes',
    },
    snapshot: {
      title: 'Rekordbox collection',
      tracks: 'tracks',
      quality: 'File health',
      setReady: 'Set ready',
      camelotKey: 'Camelot key',
      keysAnalyzed: 'keys analyzed',
      activity: 'Collection activity',
      thisMonth: 'this month',
      lastImport: 'last import',
      neverPlayed: 'never played',
      genres: 'Genre distribution',
    },
    activity: {
      title: 'Recent activity',
      empty: 'No activity this session yet.',
    },
    connections: {
      title: 'Connections',
      spotifyMeta: 'read-only · PKCE',
      connect: 'Connect',
    },
    unconfigured: {
      title: 'Almost there',
      body: 'Set your Rekordbox database and storage folder in Settings to unlock the dashboard.',
    },
  },
  activity: {
    job_sources_sync_all: 'Sources synchronized',
    job_sources_sync: 'Source synchronized',
    job_sources_apply: 'Tracks applied to Rekordbox',
    job_events_apply: 'Event applied',
    job_events_reapply: 'Event re-applied',
    job_duplicates_scan: 'Duplicates scan finished',
  },
}
